...
class CDPStatusFrame(SLFrame):
    ...
    def initialize(self):
        ...
        self.add_label_pair("", str(round( Decimal(self.dapp.pip.eth_price() / self.dapp.WAD) * self.dapp.collateral_eth_value(self.dapp.cup_id) / self.dapp.WAD, 4)) +  " USD", add_divider=False)
        self.add_button(self.close, "DEPOSIT", layout_distribution = [1, 1, 1, 1], layout_index=0)
